...
export function InsuranceProtectionSection({
  plan,
  update,
}: Props) {
  const policies =
    plan.insurancePolicies ?? [];

  const [type, setType] =
    useState<InsurancePolicy["type"]>(
      "term_life"
    );

  const [insured, setInsured] =
    useState(
      plan.clients[0]?.id ?? ""
    );

  const [benefit, setBenefit] =
    useState(0);

  const [cashValue, setCashValue] =
    useState(0);

  const [annualPremium, setAnnualPremium] =
    useState(0);
  ...
  return (
    <section className={sectionClass}>
      ...
      <p className="mb-5 text-[12px] leading-5 text-[#64748b]">
        Track life, disability, and
        long-term-care policies. Cash value
        counts toward net worth; death
        benefits feed the estate transfer
        summary; premiums can flow into
        annual expenses below.
      </p>
      ...
      <label className="mt-6 flex items-center gap-2">
        <input
          type="checkbox"
          checked={
            plan.includeInsurancePremiums ??
            true
          }
          onChange={(e) =>
            update({
              includeInsurancePremiums:
                e.target.checked,
            })
          }
        />
        ...
        <span className="text-[12px] text-[#16213e]">
          ...
          <span className="text-[#9ca3af]">
            (untick if already counted in the
            Expenses tab&apos;s
            &quot;Insurance &amp; health&quot;)
          </span>
        </span>
