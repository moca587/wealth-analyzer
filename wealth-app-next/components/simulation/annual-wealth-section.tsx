...
export function AnnualWealthSection({
  ...
  const p80 =
    result.realPercentiles["p80"] ?? [];

  const p50 =
    result.realPercentiles["p50"] ?? [];

  const p30 =
    result.realPercentiles["p30"] ?? [];
  ...
  return (
    <section className={sectionClass}>
      <h2 className={titleClass}>
        Annual Potential Wealth — to Age 90
      </h2>

      <p className="mt-2 text-[12px] leading-5 text-[#64748b]">
        Projected portfolio value each year
        from today until age 90
        (today&apos;s money), at{" "}
        ...
        likelihood — the data behind the
        chart above.
      </p>
...
const tdClass =
  "px-3 py-3 text-[11px] text-[#16213e]";
