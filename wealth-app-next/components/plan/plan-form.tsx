...
export function PlanForm({
  ...
  const [saved, setSaved] = useState<"idle" | "ok" | "err" | "conflict">(
    "idle",
  );
  ...
  const addIncome = () =>
    set({
      incomes: [
        ...plan.incomes,
        { id: newId(), clientId: plan.clients[0].id, source: "", amount: 0 },
      ],
    });
  const updateIncome = (id: string, patch: Partial<IncomeStream>) =>
    set({
      incomes: plan.incomes.map((x) => (x.id === id ? { ...x, ...patch } : x)),
    });
  const removeIncome = (id: string) =>
    set({ incomes: plan.incomes.filter((x) => x.id !== id) });

  const addExpense = () =>
    set({ expenses: [...plan.expenses, { id: newId(), name: "", amount: 0 }] });
  ...
  const removeExpense = (id: string) =>
    set({ expenses: plan.expenses.filter((x) => x.id !== id) });

  const addLoan = () =>
    set({
      loans: [
        ...plan.loans,
        { id: newId(), type: "Mortgage", bal: 0, rate: 6.5, yrs: 30 },
      ],
    });
  const updateLoan = (id: string, patch: Partial<Loan>) =>
    set({
      loans: plan.loans.map((x) => (x.id === id ? { ...x, ...patch } : x)),
    });
  const removeLoan = (id: string) =>
    set({ loans: plan.loans.filter((x) => x.id !== id) });
  ...
  const updateGoal = (id: string, patch: Partial<Goal>) =>
    set({
      goals: plan.goals.map((x) => (x.id === id ? { ...x, ...patch } : x)),
    });
  const removeGoal = (id: string) =>
    set({ goals: plan.goals.filter((x) => x.id !== id) });
  ...
  async function save() {
    ...
    try {
      ...
      if (res.status === 409) {
        ...
        setSaved("conflict");
        setSaveError(
          String(body.error ?? "This plan changed while you were editing it."),
        );
        return;
      }
      if (!res.ok) {
        ...
        renderRow={(x) => (
          <>
            <div className="space-y-1.5 sm:col-span-2">
              ...
              <Input
                value={x.source}
                onChange={(e) => updateIncome(x.id, { source: e.target.value })}
                placeholder="Salary, Bonus, Dividends…"
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) =>
                  updateIncome(x.id, {
                    amount: parseMoneyInput(e.target.value, x.amount),
                  })
                }
              />
            </div>
        ...
        renderRow={(x) => (
          <>
            <div className="space-y-1.5 sm:col-span-2">
              ...
              <Input
                value={x.name}
                onChange={(e) => updateExpense(x.id, { name: e.target.value })}
        ...
        renderRow={(x) => (
          <>
            <div className="space-y-1.5">
              ...
              <Input
                value={x.type}
                onChange={(e) => updateLoan(x.id, { type: e.target.value })}
                placeholder="Mortgage, Auto, Student…"
              />
            </div>
            <div className="space-y-1.5">
              <Label>Balance</Label>
              <Input
                ...
                onChange={(e) =>
                  updateLoan(x.id, {
                    bal: parseMoneyInput(e.target.value, x.bal),
                  })
                }
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) =>
                  updateLoan(x.id, {
                    rate: parseMoneyInput(e.target.value, x.rate),
                  })
                }
              />
            </div>
            <div className="space-y-1.5 flex items-end gap-2">
              <div className="flex-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    updateLoan(x.id, {
                      yrs: parseMoneyInput(e.target.value, x.yrs),
                    })
                  }
                />
              </div>
        ...
        renderRow={(x) => (
          <>
            <div className="space-y-1.5 sm:col-span-2">
              ...
              <Input
                value={x.name}
                onChange={(e) => updateGoal(x.id, { name: e.target.value })}
                placeholder="Retirement, Kids' college, Home purchase…"
              />
            </div>
            <div className="space-y-1.5">
              <Label>Annual amount needed</Label>
              <Input
                type="number"
                value={x.amt}
                onChange={(e) =>
                  updateGoal(x.id, {
                    amt: parseMoneyInput(e.target.value, x.amt),
                  })
                }
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) =>
                  updateGoal(x.id, {
                    startYear: parseMoneyInput(e.target.value, x.startYear),
                  })
                }
              />
            </div>
            <div className="space-y-1.5 flex items-end gap-2">
              <div className="flex-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    updateGoal(x.id, {
                      endYear: parseMoneyInput(e.target.value, x.endYear),
                    })
                  }
                />
              </div>
        ...
        <div className="text-sm max-w-xl">
          {saved === "ok" && (
            <span className="text-emerald-600">
              ...
              {saveError && (
                <span className="ml-2 text-amber-600">{saveError}</span>
              )}
            </span>
          )}
          {saved === "conflict" && (
            <span className="text-amber-600">
              {saveError} Your edits are still on screen — copy anything you
              need, then reload to merge them into the current version.
            </span>
          )}
          {saved === "err" && (
      ...
      <CardContent className="space-y-3">
        {rows.length === 0 && (
          <p className="text-sm text-muted-foreground italic">
            None yet — click Add to start.
          </p>
        )}
        {rows.map((r) => (
          <div
            key={r.id}
            className="grid sm:grid-cols-2 lg:grid-cols-5 gap-3 p-4 rounded-lg border border-border bg-muted/30"
          >
            {renderRow(r)}
          </div>
        ))}
