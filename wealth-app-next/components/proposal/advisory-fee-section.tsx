  ...
  return (
    <section className={sectionClass}>
      ...
      <p className="mt-4 text-[11px] leading-5 text-[#9ca3af]">
        Deducted from the proposal&apos;s
        gross return in the Portfolio
        Comparison simulation. Shown
        separately from the current
        portfolio fee so you can model
        the net-of-fee impact side by
        side.
      </p>
    </section>
  );
...
function formatAdvisoryFee(
  ...
  if (feeType === "aum") {
    return `${feeRate.toFixed(
      2
    )}%`;
  }

  if (feeType === "flat") {
    return `$${feeRate.toLocaleString(
      "en-US"
    )}/yr`;
  }
