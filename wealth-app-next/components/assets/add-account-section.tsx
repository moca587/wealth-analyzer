...
export function AddAccountSection({ plan, update }: Props) {
  const [type, setType] = useState(""); // account type
  const [owner, setOwner] = useState(plan.clients[0]?.id ?? "");
  const [label, setLabel] = useState("");
  const [value, setValue] = useState(0);

  const selectedOwnerClient = plan.clients.find(
    (client) => client.id === owner,
  );

  const countryCode =
    selectedOwnerClient?.country ?? plan.clients[0]?.country ?? "US";

  const countryAccounts = getCountryAccounts(countryCode);

  const accountTypes = getAccountTypes(countryCode);

  useEffect(() => {
    const typeExists = accountTypes.some((account) => account.value === type);
  ...
  //   ACCOUNT_TYPES.find((account) => account.value === type) ?? ACCOUNT_TYPES[0];
  const selectedAccount =
    accountTypes.find((account) => account.value === type) ?? accountTypes[0];
  ...
  function addAccount() {
    ...
    const newAsset = {
      ...
      label: label.trim() || selectedAccount.label,
      ...
      withdrawAge: selectedAccount.withdrawAge,

      note: selectedAccount.note,

      owner,

      // country: plan.clients[0]?.country,
      country: countryCode,
  ...
  return (
    <section className={sectionClass}>
      <h2 className={titleClass}>Add Account</h2>

      <div className="mb-4 text-[11px] text-[#9ca3af]">
        {countryAccounts.flag} {countryAccounts.name} account types
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label>
          ...
          <select
            ...
            className={inputClass}
          >
            {countryAccounts.groups.map((group) => (
        ...
        <label>
          ...
          >
            ...
            {plan.clients.length > 1 && <option value="joint">Joint</option>}
          </select>
        </label>
        ...
        <label>
          <span className={labelClass}>Balance / Value</span>
