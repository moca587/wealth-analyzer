      ...
      <Card>
        ...
        <CardContent className="flex flex-wrap items-end gap-4">
          <div className="space-y-1.5">
            ...
            <Select
              value={sims}
              onChange={(e) => setSims(+e.target.value as 200 | 500 | 1000)}
              className="w-32"
            >
      ...
      {result && (
        <>
          <Card>
            <CardHeader>
              <CardTitle>
                Net worth projection — {sims.toLocaleString()} paths,{" "}
                {result.years} years
              </CardTitle>
            </CardHeader>
            <CardContent>
          ...
          {result.retirement &&
            (() => {
              ...
              return (
                <Card>
                  <CardHeader>
                    <CardTitle>Will your money last?</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="flex items-center gap-6 flex-wrap">
                      <div>
                        ...
                        <div className={`font-display text-4xl ${tone}`}>
                          {(succ * 100).toFixed(0)}%
                        </div>
                      </div>
                      <div className="flex-1 min-w-[220px]">
                        <div className="h-3 rounded-full bg-muted overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, Math.max(0, succ * 100))}%`,
                            }}
                          />
                        </div>
                        <div className="text-xs text-muted-foreground mt-2">
                          Retiring at age {result.retirement.retirementAge},
                          modelled through age {result.retirement.planToAge}.{" "}
                          {(
                            result.retirement.depletionProbability * 100
                          ).toFixed(0)}
                          % of scenarios run the portfolio to zero before then.
                        </div>
                      </div>
                    </div>
                  </CardContent>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {[
              {
                k: "Median (P50)",
                v: result.realFinal.p50,
                cls: "text-foreground",
              },
              {
                k: "P25",
                v: result.realFinal.p25,
                cls: "text-muted-foreground",
              },
              {
                k: "P75",
                v: result.realFinal.p75,
                cls: "text-muted-foreground",
              },
              {
                k: "P10 (worst)",
                v: result.realFinal.p10,
                cls: "text-destructive",
              },
              {
                k: "P90 (best)",
                v: result.realFinal.p90,
              ...
              <Card key={s.k}>
                <CardContent className="p-4">
                  ...
                  <div className={`font-display text-2xl ${s.cls}`}>
                    {formatMoney(s.v, plan.currency)}
                  </div>
                </CardContent>
              </Card>
            ))}
          ...
          {result.goalSuccess.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle>Goal probability of success</CardTitle>
              </CardHeader>
                  ...
                  <div key={g.goalId}>
                    <div className="flex items-baseline justify-between mb-1.5">
                      <span className="font-medium">
                        {g.goalName || "(unnamed goal)"}
                      </span>
                      <span
                        className={`font-display text-lg ${g.probability >= 0.8 ? "text-emerald-600" : g.probability >= 0.5 ? "text-amber-500" : "text-destructive"}`}
                      >
                    ...
                    <div className="h-2 rounded-full bg-muted overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.min(100, Math.max(0, g.probability * 100))}%`,
                        }}
                      />
                    </div>
                  </div>
          ...
          <div className="text-xs text-muted-foreground border-t border-border pt-4">
            <strong>Important:</strong> Monte Carlo results are illustrative
            projections based on the inputs you provided and assumed risk/return
            parameters. They are not predictions, guarantees, or financial, tax,
            or investment advice. Tax figures are simplified estimates — this
            tool does not provide tax advice. Real outcomes will differ. Consult
            a licensed advisor and a tax professional before making decisions.
          </div>
        </>
      )}
    </div>
