...
export function WealthOverviewSection({ plan }: { plan: WealthPlan }) {
  ...
  const annualSurplus =
    grossIncome - annualTax - annualExpenses - annualDebtService;

  const monthlyCashflow = annualSurplus / 12;

  const savingsRate =
    grossIncome > 0 ? (annualSurplus / grossIncome) * 100 : null;
    ...
    <section className="mb-6 flex min-h-[290px] overflow-hidden rounded-xl bg-gradient-to-br from-white to-[#f0f5ff] shadow-[0_1px_4px_rgba(0,0,0,.05),0_6px_20px_rgba(0,87,184,.07)]">
      <div className="flex flex-1 flex-col px-9 py-8">
        ...
        <div className="grid grid-cols-2 gap-2.5">
          ...
          <Metric
            label="Debt-to-Asset"
            value={debtToAsset !== null ? `${debtToAsset.toFixed(1)}%` : "—"}
          />
          <Metric
            label="Projected Wealth (median)"
            value={
              result ? formatMoney(result.realFinal.p50, plan.currency) : "—"
            }
          />{" "}
        </div>
      </div>

      <div className="flex w-[300px] flex-col items-center justify-center gap-[18px] border-l border-[rgba(0,87,184,.07)] bg-[#eaf1ff] px-6 py-7">
        {" "}
        <div className="text-[10px] font-bold uppercase tracking-[0.10em] text-[#9ca3af]">
          Asset Allocation
        </div>
        <AssetAllocationDonut assets={plan.assets} currency={plan.currency} />
        {totalAssets === 0 && (
