...
export function ReportView({ plan }: { plan: WealthPlan }) {
  ...
  const sum = <T,>(a: T[], g: (x: T) => number) =>
    a.reduce((s, x) => s + (g(x) || 0), 0);
    ...
    <div className="report-root">
      ...
      <div className="report-noprint flex items-center justify-between mb-6">
        <div className="text-sm text-muted-foreground">
          Review below, then export a clean PDF.
        </div>
      ...
      <div className="mx-auto max-w-3xl space-y-6 print:space-y-0">
        ...
        <section className="report-page bg-white text-slate-900 rounded-xl border border-slate-200 shadow-sm p-12 min-h-[60vh] flex flex-col justify-between">
          ...
          <div>
            ...
            <p className="text-slate-500 max-w-md">
              A Monte Carlo projection of your household&apos;s position,
              portfolio, goals, and — if enabled — retirement sustainability.
            </p>
          </div>
        ...
        <section className="report-page bg-white text-slate-900 rounded-xl border border-slate-200 shadow-sm p-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4">
            ...
            <Stat
              label="Projected wealth (median, 30y)"
              value={m(sim.final.p50)}
            />
            {sim.retirement && (
              <Stat
                label="Money-lasts probability"
                value={pct(sim.retirement.successProbability)}
              />
            )}
          </div>
          <p className="text-sm text-slate-500 mt-6 leading-relaxed report-avoid">
            Over a 30-year, 1,000-path Monte Carlo simulation, your net worth is
            projected to reach a median of{" "}
            <strong className="text-slate-800">{m(sim.final.p50)}</strong>{" "}
            (10th–90th percentile {m(sim.final.p10)} – {m(sim.final.p90)}).{" "}
            {sim.retirement
        ...
        <section className="report-page bg-white text-slate-900 rounded-xl border border-slate-200 shadow-sm p-10">
          ...
          <table className="w-full text-sm mt-4">
            <thead>
              <Tr
                head
                cells={["Client", "Age", "Country", "Risk profile", "Horizon"]}
              />
            </thead>
            <tbody>
              {plan.clients.map((c) => (
          ...
          {plan.children.length > 0 && (
            <p className="text-sm text-slate-500 mt-4">
              ...
              {plan.children
                .map(
                  (c) =>
                    `${[c.first, c.last].filter(Boolean).join(" ")}${c.dob ? ` (age ${ageFromDOB(c.dob)})` : ""}`,
                )
                .join(", ")}
              .
            </p>
        ...
        <section className="report-page bg-white text-slate-900 rounded-xl border border-slate-200 shadow-sm p-10">
          ...
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 mt-5 mb-2">
            Assets by class
          </h3>
                ...
                {classRows.map(([cls, v]) => (
                  <tr key={cls} className="border-b border-slate-100">
                    ...
                    <td className="py-1.5 text-right tabular-nums text-slate-500">
                      {totalAssets > 0 ? pct(v / totalAssets) : "—"}
                    </td>
                  </tr>
                ))}
                <Tr
                  strong
                  cells={["Total assets", m(totalAssets), "100%"]}
          ...
          <table className="w-full text-sm">
            <tbody>
              ...
              <Tr
                strong
                cells={["Total liabilities", "", m(totalLiab)]}
                align={["left", "left", "right"]}
              />
            </tbody>
          </table>
        ...
        <section className="report-page bg-white text-slate-900 rounded-xl border border-slate-200 shadow-sm p-10">
          ...
          <table className="w-full text-sm mt-4">
            <tbody>
              <Tr
                cells={["Gross annual income", m(grossIncome)]}
                align={["left", "right"]}
              />
              <Tr
                cells={["Estimated income tax", `(${m(tax)})`]}
                align={["left", "right"]}
              />
              <Tr
                cells={["Annual expenses", `(${m(annualExpense)})`]}
                align={["left", "right"]}
              />
              {annualDebtService > 0 && (
                <Tr
                  cells={[
                    "Debt service (loans, first year)",
                    `(${m(annualDebtService)})`,
                  ]}
                  align={["left", "right"]}
                />
              ...
              <Tr
                strong
                cells={["Annual surplus", m(annualSurplus)]}
                align={["left", "right"]}
              />
            </tbody>
          </table>
          <p className="text-xs text-slate-400 mt-3">
            Income tax is a simplified, federal-level estimate for{" "}
            {plan.clients[0]?.country || "US"}; it excludes state/local layers
            and credits. It is not tax advice — consult a tax professional.
            Expense categories should exclude payments on tracked loans — debt
            service is shown (and simulated) separately.
          </p>
        </section>
        ...
        <section className="report-page bg-white text-slate-900 rounded-xl border border-slate-200 shadow-sm p-10">
          ...
          <p className="text-sm text-slate-500 mb-4">
            Net worth across {sim.sims.toLocaleString()} simulated market paths
            over {sim.years} years (today&apos;s dollars).
          </p>
          <div className="report-avoid">
            <SimChart result={sim} currency={ccy} />
          </div>
            ...
            <div className="flex items-center gap-8 mt-4 report-avoid">
              <div>
                ...
                <div className="font-display text-5xl text-primary">
                  {pct(sim.retirement.successProbability)}
                </div>
              </div>
              <p className="text-sm text-slate-500 flex-1">
                Retiring at age {sim.retirement.retirementAge}, modelled through
                age {sim.retirement.planToAge}.{" "}
                {pct(sim.retirement.depletionProbability)} of scenarios exhaust
                the portfolio before then. Pensions are credited (with COLA) and
                RMDs are taken from tax-deferred accounts past age 73.
              </p>
            </div>
          </section>
        )}
        ...
        <section className="report-page bg-white text-slate-900 rounded-xl border border-slate-200 shadow-sm p-10">
          <SectionTitle
            n={sim.retirement ? "08" : "07"}
            title="Methodology & disclosures"
          />
          <ul className="text-sm text-slate-600 mt-4 space-y-2 list-disc pl-5 report-avoid">
            <li>
              Returns are simulated as log-normal draws. Portfolio expected
              return and volatility are derived from your actual allocation
              using per-asset-class capital-market assumptions and a correlation
              matrix (so diversification reduces risk).
            </li>
            <li>
              Expenses and goals are inflated at {pct(plan.inflationRate)} per
              year; income is taxed using simplified federal brackets.
            </li>
            <li>
              Loans amortize with real interest math, and full debt service
              (interest + principal) is paid from each year&apos;s cash flow —
              expense categories are assumed to exclude those payments. Property
              appreciates stochastically and is not liquidated for spending.
            </li>
            {sim.retirement && (
              <li>
                In retirement, salary stops, spending is drawn from taxable then
                tax-deferred accounts, pensions are credited, and RMDs are
                forced past age 73.
              </li>
            )}
          </ul>
          <p className="text-xs text-slate-400 mt-6 leading-relaxed report-avoid">
            <strong>Important:</strong> This report is an illustrative
            projection based on the inputs provided and assumed capital-market
            parameters. It is not a prediction, guarantee, or personalized
            financial, tax, or investment advice. Actual results will differ.
...
function Stat({
  ...
  return (
    <div className="report-avoid">
      ...
      <div className={`font-display ${big ? "text-3xl" : "text-xl"}`}>
        {value}
      </div>
    </div>
  );
...
function Tr({
  ...
  return (
    <tr
      className={`${head ? "text-[10px] font-bold uppercase tracking-wider text-slate-400" : ""} ${strong ? "font-semibold border-t border-slate-200" : ""} border-b border-slate-100 last:border-0`}
    >
      {cells.map((c, i) => (
        <td
          key={i}
          className={`py-2 ${(align?.[i] ?? (i === 0 ? "left" : "left")) === "right" ? "text-right" : "text-left"} ${head ? "" : "text-slate-700"} tabular-nums`}
        >
          {c}
        </td>
      ))}
