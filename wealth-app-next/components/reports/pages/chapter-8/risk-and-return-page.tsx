"use client";

import { ReportPage } from "../../report-page";
import { EducationBarChart } from "../../charts/education-bar-chart";

type Props = {
  clientName: string;
  date: string;
  page: number;
  totalPages: number;
};

const chartItems = [
  {
    label: "Conservative",
    value: 4.5,
    color: "#0867b9",
  },
  {
    label: "Moderate",
    value: 6.5,
    color: "#0ea5a8",
  },
  {
    label: "Growth",
    value: 8.5,
    color: "#c8941f",
  },
  {
    label: "Aggressive",
    value: 9.5,
    color: "#00875a",
  },
];

export function RiskAndReturnPage({
  clientName,
  date,
  page,
  totalPages,
}: Props) {
  return (
    <ReportPage
      clientName={clientName}
      title="Investor Education"
      date={date}
      page={page}
      totalPages={totalPages}
    >
      <div className="mt-3 border-l-[4px] border-[#0867b9] bg-[#dceaf7] px-3 py-2 text-[13px] font-bold text-[#173d60]">
        Risk and return go together
      </div>

      <p className="mt-4 text-[9px] text-[#6b7280]">
        Expected annual return by model allocation (higher return, higher
        volatility)
      </p>

      <div className="mt-6 grid grid-cols-[58%_42%] gap-8">
        <div className="h-[255px]">
          <EducationBarChart items={chartItems} format="percent" />
        </div>

        <div className="pt-2">
          <ul className="space-y-4 text-[9px] leading-[1.55] text-[#30343b]">
            <Bullet>
              Higher expected returns come with wider swings along the way
              (higher volatility).
            </Bullet>

            <Bullet>
              The right allocation depends on your time horizon and tolerance
              for short-term loss.
            </Bullet>

            <Bullet>
              Longer horizons can support more growth assets because there is
              more time to recover.
            </Bullet>
          </ul>
        </div>
      </div>

      <p className="mt-7 text-[7px] italic leading-[1.45] text-[#6b7280]">
        Illustrative expected returns based on long-run capital market
        assumptions. Not a forecast; actual returns will vary.
      </p>
    </ReportPage>
  );
}

function Bullet({ children }: { children: React.ReactNode }) {
  return (
    <li className="flex gap-3">
      <span className="mt-[5px] h-[5px] w-[5px] shrink-0 rounded-full bg-[#0867b9]" />
      <span>{children}</span>
    </li>
  );
}
