...
- **Undo notice (done)** (`showUndoToast(msg, onUndo)`, `#undo-toast`): navy pill, bottom-centre, message + `undo-2` **Undo** + dismiss `x`. It auto-dismisses after 8s, and a new one replaces the old. Use it after a destructive single-row action instead of a confirm dialog. `onUndo` may return a string to explain why undo isn't possible (the Undo button is then removed). Used by: removing a holding on Current Portfolio, and removing a goal. A goal's Undo refuses once another profile has been loaded (`window._profileGen`, bumped by `restoreProfile`). It restores the holding into the portfolio it came from, at the same position, and refuses if that portfolio was deleted or another profile was loaded.
...
- **Emoji** are used only for country flags (🇨🇭, the AI Builder's country banner, the retirement-location list including its 🌍 "Other"). Flags stay emoji and are never swapped for an icon. They are content, not part of the icon set, and never the only label.
...
- **Current Portfolio tab (done):** Holdings header buttons and their sub-panel titles use `plug` (Data feeds), `file-down` (Import statement) and `sparkles` (AI Data Agent). *Refresh live prices* and the ticker *Fetch* button (also on Investment Proposal) use `refresh-cw`. *Cancel* uses `x`. The AI-key notes (AI Data Agent and the Upload Document modal) and the holdings reconcile line use `circle-check` / `triangle-alert`. The ✎ / ✕ row buttons are the shared `.btn-x` pattern and are unchanged.
- **Goal categories (done):** `GOAL_ICONS[cat].icon` is a Lucide name (`tree-palm`, `house`, `graduation-cap`, `plane`, `shield-check`, `briefcase`, `gem`, `car`, `heart-pulse`, `gift`, `star`) drawn in the category colour inside `.goal-icon`, on the Goals list and the Simulation goal-funding list. The category `<select>` options are plain text. The cash-flow *savings constrained* marker is `triangle-alert` (`.cf-warn`), which also keeps the PDF's cell text free of a glyph its fonts can't draw.
- **Row actions (done, Current Portfolio holdings + Goals):** one `.row-actions` cell at the end of the row with 28px outline icon buttons (`.row-act`, `.row-act-del`): `pencil` (Edit) and `trash-2` (Remove), each with `title` + `aria-label`. Remove turns red on hover or focus. This replaces the ✎/✕ pair, where ✕ was an 11th item in a 10-column grid and wrapped under the Type badge (since the first commit). Other lists still use `.btn-x` ✕.
